...
public class AlbumActivity extends Activity {
	...
	public static Class<?> cls;// 传入点击的界面
	...
	private class AlbumSendListener implements OnClickListener {
		public void onClick(View v) {
			...
			intent.putExtra("num", Bimp.tempSelectBitmap.size());
			if (Bimp.tempSelectBitmap.size() > 0) {
				Bundle bundleObject = new Bundle();
				// bundleObject.putSerializable("imglist",
				// Bimp.tempSelectBitmap);
				// intent.putExtras(bundleObject);
				intent.putExtra("imglist", Bimp.tempSelectBitmap);
				startActivity(intent);
			}
			startActivity(intent);
			finish();
		...
		gridImageAdapter
				.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
					...
					public void onItemClick(final ToggleButton toggleButton,
						...
						if (Bimp.tempSelectBitmap.size() >= PublicWay.num) {// 判断如果超出5张
																			// 并且是5张不重复的就Toast提示
							toggleButton.setChecked(false);
							chooseBt.setVisibility(View.GONE);
							if (!removeOneData(dataList.get(position))) {// 如果选择器里面不包含当前图片
						...
						if (isChecked) {
							chooseBt.setVisibility(View.VISIBLE);// 选择了就显示被选择的图标
							Bimp.tempSelectBitmap.add(dataList.get(position));
							okButton.setText(Res.getString("finish") + "("
									+ Bimp.tempSelectBitmap.size() + "/"
									+ PublicWay.num + ")");// 更新已选择图片的个数
						} else {
