...
public class SmallSingleAnnularUI extends View{
	...
	public SmallSingleAnnularUI(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		//获取颜色属性
		TypedArray tArray = context.obtainStyledAttributes(attrs,
				R.styleable.SmallSingleAnnularUI);
		colornum = tArray.getString(R.styleable.SmallSingleAnnularUI_uiclocor);
		init();
	}
	
	public SmallSingleAnnularUI(Context context, AttributeSet attrs) {
		super(context, attrs);
		//获取颜色属性
		TypedArray tArray = context.obtainStyledAttributes(attrs,
				R.styleable.SmallSingleAnnularUI);
	...
	public SmallSingleAnnularUI(Context context) {
		super(context);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		mbackgroundrectf.right  = dip2px(getContext(), 62.5f);                      
		mbackgroundrectf.bottom = dip2px(getContext(), 62.5f); 
		//画背景粗线圆弧
		...
		if(score != 0){
			...
			//计算小圆中心点坐标
			double startx = dip2px(getContext(), 32.5f) - (dip2px(getContext(), 30f) * Math.sin(Math.toRadians(xitastart)));
			double starty = dip2px(getContext(), 32.5f) - (dip2px(getContext(), 30f) * Math.cos(Math.toRadians(xitastart)));
			...
			mrectf.right  = dip2px(getContext(), 62.5f);                      
			mrectf.bottom = dip2px(getContext(), 62.5f); 
			//画粗线圆弧
			canvas.drawArc(mrectf, 270 - xitaend, xitaend - xitastart, false, mpaint); 
			
			
			//计算小圆中心点坐标
			double endx = dip2px(getContext(), 32.5f) - (dip2px(getContext(), 30f) * Math.sin(Math.toRadians(xitaend)));
			double endy = dip2px(getContext(), 32.5f) - (dip2px(getContext(), 30f) * Math.cos(Math.toRadians(xitaend)));
		...
		canvas.drawText((int)score+"", dip2px(getContext(), 32.5f), dip2px(getContext(), 38f), textpaint);
