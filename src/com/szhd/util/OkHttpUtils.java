...
/**
 * * OkHttp框架的二次封装 具有以下功能： 1、下载解析json数据，可根据指定的类型自动转换为相应的实体类或数组 2、上传文件
 * 3、下载文件，当下载文件直接由服务端读写，则可以实现更新下载进度的效果。 没有图片下载、二级缓存的功能。
 * 
 * Created by yao on 2016/5/16. 1.增加两个重载的downloadFile方法，一个用于更新下载进度，一个不更新下载进度。
 * 2.在initHandler方法中增加对下载状态的响应。 3.在execute方法中增加对url中request的检查。
 */
public class OkHttpUtils<T> {
	...
	/**
	 * 执行本构造方法后，别忘了在当前组件的onDestroy方法中调用 OkHttpUtils.release()
	 */
	public OkHttpUtils() {
	...
	/**
	 * 设置工作线程中执行的代码，如文件下载。 用于代替缺省的下载解析json的execute中的Callback
	 * 
	 * @param callback
	 *            :工作线程中执行的代码
	 * @return
	...
	/**
	 * execute中执行的工作线程中的代码， 调用本方法后，exeucute方法中的参数应设置为null
	 * 
	 * @param listener
	 *            :保存处理服务端返回结果的代码块
	 */
